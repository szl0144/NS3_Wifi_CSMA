                           ...
                           MacLowTransmissionListener *listener)
{
  ...
  NS_LOG_WARN ( "  startTx  size = " << GetSize (m_currentPacket, &m_currentHdr)                  
                << ", to=" << m_currentHdr.GetAddr1 () << "  from = "<< m_currentHdr.GetAddr2 ()
                << "    " << Simulator ::Now () 
                << ",  listener=" << m_listener);
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
  ...
  NS_LOG_WARN ("  ReceiveOk  size = "  << packet->GetSize ()
               <<  "  hdr.GetAddr1 () = " << hdr.GetAddr1 ()
               << "    " << Simulator ::Now ()                
