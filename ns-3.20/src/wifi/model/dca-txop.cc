    ...
    m_currentPacket (0)
{
  ...
  m_eng = new RealRandomStreamex ();
  //m_eng = new RealExponentialRandomStream ();
...
DcaTxop::RestartAccessIfNeeded (void)
{
  ...
  if ((m_currentPacket != 0
    ...
    {
      NS_LOG_WARN ("  At time " << Simulator ::Now() 
...
DcaTxop::DoInitialize ()
{
  NS_LOG_FUNCTION (this);
  m_dcf->ResetCw ();
  //NS_LOG_WARN ("mean of my exponential r.v. is = " << mean << ":");
  //NS_LOG_WARN ("mean of DoInitialize exponential r.v. is = " << mean << 
  //                                ":    m_dcf->GetCw = " << m_dcf->GetCw ());
  //m_dcf->StartBackoffNow (m_eng->GetNext (mean, m_dcf->GetCw ()));
  //NS_LOG_WARN ("DoInitialize");
  m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
...
DcaTxop::NotifyCollision (void)
{
  ...
  //m_dcf->StartBackoffNow (m_eng->GetNext (0, m_dcf->GetCw ()));
  //NS_LOG_WARN ("mean of my exponential r.v. is = " << mean << ":");
  //NS_LOG_WARN ("At time  " << Simulator::Now ()  << " mean of collision Our exponential r.v. is = " << mean << 
  //              ":    m_dcf->GetCw = " << m_dcf->GetCw ());
  //m_dcf->StartBackoffNow (m_eng->GetNext (mean, m_dcf->GetCw ()));
  NS_LOG_WARN ("  At time " << Simulator ::Now() 
                <<"  NotifyCollision");
  uint32_t num_1 = m_eng->GetNext (mean,0);
  //m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
  NS_LOG_WARN ("  At time " << Simulator ::Now() 
                <<"  nSlots = " << num_1 << " ");
  m_dcf->StartBackoffNow (num_1);
  RestartAccessIfNeeded ();
}
...
DcaTxop::MissedCts (void)
{
  ...
  //m_dcf->StartBackoffNow (m_eng->GetNext (0, m_dcf->GetCw ()));
  //NS_LOG_WARN ("mean of my exponential r.v. is = " << mean << ":");
  //NS_LOG_WARN ("At time  " << Simulator::Now ()  <<"mean of MissedCts exponential r.v. is = " << mean << 
  //                                ":    m_dcf->GetCw = " << m_dcf->GetCw ());
  //m_dcf->StartBackoffNow (m_eng->GetNext (mean, m_dcf->GetCw ()));
  //NS_LOG_WARN ("MissedCts");
  m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
  RestartAccessIfNeeded ();
}
void
DcaTxop::GotAck (double snr, WifiMode txMode)
{
  ...
  if (!NeedFragmentation ()
      || IsLastFragment ())
    {
      ...
      //m_dcf->StartBackoffNow (m_eng->GetNext (0, m_dcf->GetCw ()));
      //NS_LOG_WARN ("mean of my exponential r.v. is = " << mean << ":");
      //NS_LOG_WARN ("mean of GotAck exponential r.v. is = " << mean << 
      //                              ":    m_dcf->GetCw = " << m_dcf->GetCw ());
      //m_dcf->StartBackoffNow (m_eng->GetNext (mean, m_dcf->GetCw ()));
      //NS_LOG_WARN ("GotAck");
      m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
...
DcaTxop::MissedAck (void)
{
  ...
  //m_dcf->StartBackoffNow (m_eng->GetNext (0, m_dcf->GetCw ()));
  //NS_LOG_WARN ("mean of my exponential r.v. is = " << mean << ":");
  //NS_LOG_WARN ("At time  " << Simulator::Now ()  << "mean of MissedAck exponential r.v. is = " << mean << 
  //                                ":    m_dcf->GetCw = " << m_dcf->GetCw ());
  //m_dcf->StartBackoffNow (m_eng->GetNext (mean, m_dcf->GetCw ()));
  //NS_LOG_WARN ("MissedAck");
  m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
  RestartAccessIfNeeded ();
}
void
...
DcaTxop::EndTxNoAck (void)
{
  ...
  m_currentPacket = 0;
  m_dcf->ResetCw ();
  //NS_LOG_WARN ("At time  " << Simulator::Now ()  << "mean of EndTxNoAck exponential r.v. is = " << mean << 
  //                                  ":    m_dcf->GetCw = " << m_dcf->GetCw ());
  //NS_LOG_WARN ("EndTxNoAck");
  m_dcf->StartBackoffNow (m_eng->GetNext (mean, 0));
  StartAccessIfNeeded ();
}
