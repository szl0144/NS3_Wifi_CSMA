...
DcfState::StartBackoffNow (uint32_t nSlots)
{
  ...
  //printf(" collision happens initt\n");
}
...
DcfManager::RequestAccess (DcfState *state)
{
  ...
  if (state->GetBackoffSlots () == 0
      && IsBusy ())
    {
       ...
       * generate a backoff.
       */
      //NS_LOG_WARN ("  At time "<< Simulator ::Now () << "  medium is busy: collision");
      state->NotifyCollision ();
      /*NS_LOG_WARN ( "  At time " << Simulator ::Now () <<  "  Real Backofftime is =" 
                    << MicroSeconds((state->GetBackoffSlots ())*m_slotTimeUs) 
                    << "  m_slotTimeUs " << m_slotTimeUs 
                    <<  "  state->GetBackoffSlot "  << state->GetBackoffSlots () );  */
      //NS_LOG_WARN ( "Real Backofftime is =" << MicroSeconds((state->GetBackoffSlots ())*slotTime.GetMicroSeconds ()) << " " );  
      //(Simulator::Now () - backoffStart).GetMicroSeconds ();
    }
  DoGrantAccess ();
  DoRestartAccessTimeoutIfNeeded ();
  //NS_LOG_WARN ( "  At time " << Simulator ::Now () <<  "  !! Request Access !! Real Backofftime is =" 
  //              << MicroSeconds((state->GetBackoffSlots ())*m_slotTimeUs) 
  //              << "  m_slotTimeUs " << m_slotTimeUs 
  //              <<  "  state->GetBackoffSlot "  << state->GetBackoffSlots () ); 
}
  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++, k++)
    {
      ...
      //NS_LOG_WARN ("  At time " << Simulator ::Now() 
      //            << "  UpdateBackoff = " <<  state->GetBackoffSlots ()  << " ");
...
DcfManager::DoRestartAccessTimeoutIfNeeded (void)
{
  ...
  //NS_LOG_WARN ("  expectedBackoffEnd initial = " << expectedBackoffEnd  <<  " ");
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); i++)
    ...
    //NS_LOG_WARN ("At time " << Simulator ::Now() 
    //              << "  GetBackoffEndFor (state) = " << expectedBackoffEnd << " ");
...
{
  NS_LOG_FUNCTION (this << duration);
  //NS_LOG_WARN ("At time " <<Simulator::Now () << "   NotifyMaybeCcaBusyStartNow = " << duration);
...
DcfManager::NotifyNavResetNow (Time duration)
{
  ...
  //NS_LOG_WARN ("At time " <<Simulator::Now () << "   NotifyNavResetNow = " << duration);
  UpdateBackoff ();
...
DcfManager::NotifyAckTimeoutStartNow (Time duration)
{
  ...
  //NS_LOG_WARN ("  At time " << Simulator ::Now () 
  //              <<  "  NotifyAckTimeoutStartNow Duration is " << duration  << " " );
